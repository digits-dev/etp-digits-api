    ...
    <div class='panel panel-default'>
        ...
        <form action="{{ route('update-approval-matrix') }}" method="POST" id="userForm" enctype="multipart/form-data">
                ...
                <div class="container">
                    <div class="row">
                        <div class="form-group">
                            ...
                            <div class="col-sm-5">
                                <select data-placeholder="** Please select a Privilege" class="form-control select2" id="privilege" name="privilege" required>
                                    <option></option>
                                    @foreach($privileges as $priv)
                                        <option value="{{ $priv->id }}" {{ $approval_matrix->cms_privileges_id == $priv->id ? 'selected' : '' }}>{{ $priv->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                    ...
                    <div class="row">
                        <div class="form-group">
                            ...
                            <div class="col-sm-5">
                                <select data-placeholder="** Please select an Approver" class="form-control select2" id="approver_id" name="approver_id" required>
                    ...
                    <div class="row">
                        <div class="form-group">
                            ...
                            <div class="col-sm-5">
                                <select data-placeholder="** Please select a Channel" class="form-control select2" id="channel" name="channels_id" required>
                                    <option></option>
                                    @foreach($channels as $chanel)
                                        <option value="{{ $chanel->id }}" {{ $approval_matrix->channel_id == $chanel->id ? 'selected' : '' }}>{{ $chanel->channel_description }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
            ...
            <div class='panel-footer'>
                ...
                <button class="btn btn-success" type="submit" id="btnSubmit"> <i class="fa fa-save"></i> Save Changes</button>
            </div>
        </form>
    </div>
@endsection
        ...
        $(document).ready(function() {
            ...
            function loadApprovers(privilegeId, selectedApproverId) {
                $.ajax({
                    ...
                    type: "POST",
                    data: { privilege_id: privilegeId, _token: "{{ csrf_token() }}" },
                    success: function (data) {
                        $('#approver_id').empty().append('<option></option>');
                        $.each(data, function (key, value) {
                            let selected = key == selectedApproverId ? 'selected' : '';
                            $('#approver_id').append(`<option value="${key}" ${selected}>${value}</option>`);
                        });
                    }
                });
            }
            ...
            function loadStores(channelId, selectedStoreIds) {
                $.ajax({
                    url: "{{ route('getStores') }}",
                    type: "POST",
                    data: { channel_id: channelId, _token: "{{ csrf_token() }}" },
                    success: function (data) {
                        $('#store_ids').empty().append('<option value="all">All</option>');
                        $.each(data, function (key, value) {
                            let selected = selectedStoreIds.includes(key.toString()) ? 'selected' : '';
                            $('#store_ids').append(`<option value="${key}" ${selected}>${value}</option>`);
                        });
                    }
                });
            }
